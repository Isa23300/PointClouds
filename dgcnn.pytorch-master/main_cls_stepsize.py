...
def train(args, io):
    ...
    train_loader = DataLoader(MyShape(root=DATA_PATH, npoint=1024, split='train'), num_workers=1,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(MyShape(root=DATA_PATH, npoint=1024, split='test'), num_workers=1,
...
def parse_args(x):
    ...
    #parser.add_argument('--exp_name', type=str, default='stepsize_' + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')),metavar='N',help='Name of the experiment')

    parser.add_argument('--exp_name', type=str, default='lr_test',metavar='N',help='Name of the experiment')
    ...
    parser.add_argument('--use_sgd', type=bool, default=True,help='Use Adam')
    parser.add_argument('--lr', type=float, default=x, metavar='LR',help='learning rate (default: 0.001, 0.1 if using sgd)')
...
if __name__ == "__main__":
    list_lr = [0.001, 0.0001, 0.00015, 0.00001, 0.000015, 0.000001, 0.000005]
    for x in list_lr:
        args = parse_args(x)
        main(args)
