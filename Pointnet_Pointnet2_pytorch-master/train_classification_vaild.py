...
def parse_args(x):
    ...
    parser.add_argument('--filename', type=str, default='lr_'+str(x)+'_'+str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')), help='use cpu mode')
    ...
    parser.add_argument('--learning_rate', default=x, type=float, help='learning rate in training')
...
if __name__ == '__main__':
    #list_gamma=[0.001, 0.0001, 0.00015, 0.00001, 0.000015, 0.000001, 0.000005]

    list_gamma=[0.0005,0.00005]
    for x in list_gamma:
        args = parse_args(x)
        main(args)
