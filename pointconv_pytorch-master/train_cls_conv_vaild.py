...
def parse_args(x):
    ...
    parser.add_argument('--batchsize', type=int, default=x, help='batch size in training')
    ...
    parser.add_argument('--gamma', type=float, default=0.7, help='StepLR,default=0.7')
...
def main(args):
    ...
    file_dir = Path(str(experiment_dir) + '/bs'+str(args.batchsize)+'-'+ str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
...
#_____________test____________________________________________
        ...
        with torch.no_grad():
            ...
            outstr = 'Test %d, test acc: %.6f, test avg acc: %.6f, loss: %6f' % \
                     (epoch,acc,metrics.balanced_accuracy_score(test_true, test_pred),test_loss)
            logger.info(outstr)
            print(outstr)
    #____________________________________________________________________________
        if (acc >= best_tst_accuracy) and epoch > 10:
            ...
            save_simple_checkpoint(
                ...
                optimizer,
                str(checkpoints_dir),
                args.model_name+str(args.learning_rate))
            print('Saving model....')
...
if __name__ == '__main__':
    #list_num=[0.001, 0.0001, 0.00015, 0.00001, 0.000015, 0.000001, 0.000005]
    #list_num=[0.1, 0.01]
    list_num=[4,8,16]
    for x in list_num:
        args = parse_args(x)
